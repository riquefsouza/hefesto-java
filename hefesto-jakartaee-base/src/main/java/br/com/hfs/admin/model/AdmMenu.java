...
@NamedQueries({
	...
	@NamedQuery(name = "AdmMenu.findMenuRoot", query = "SELECT m FROM AdmMenu m left join m.admMenuParent mp left join m.admPage f WHERE m.admMenuParent IS NULL ORDER BY m.order, mp.order"),
	@NamedQuery(name = "AdmMenu.findMenuRootByDescription", query = "SELECT m FROM AdmMenu m left join m.admMenuParent mp left join m.admPage f WHERE m.admMenuParent IS NULL AND m.description = ?1 ORDER BY m.order, mp.order"),
